    ...
    with gr.Blocks(css=css) as demo:
        ...
        with gr.Column(visible=True) as start_col:
            gr.Markdown(
                ...
                "<p align='center' style='width: 70%; margin: 0 auto;'>"
                "In this evaluation, you will compare shorter audio samples generated by different TTS systems to ground truth speech as the reference. "
                "Speakers and their audio samples are randomly selected. For each speaker, a sample from the original data and five samples "
                "as the reference audio for voice cloning, and the other as the text prompt for generating the synthetic sample. "
                "You will evaluate the generated sample against this generated synthetic sample. "
                "Assess each sample for clarity, accuracy, and similarity to the reference by providing scores for SMOS (speaker similarity), "
                "CMOS (comparative naturalness), and Intelligibility (naturalness, robotic voice) scores."
                "</p>"
                "<hr>"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_samples', default=14, help="Number of samples to compare. Defaults to 5.")
    parser.add_argument('--group', default=1, help="Group of evaluators, 1-3")
    args = parser.parse_args()

    dataset_paths_short = [
        "generated_speech/GPT_XTTS_v2.0_Full_7_5/snf_short_dataset",
        "generated_speech/GPT_XTTS_v2.0_Full_7_6_SNF/snf_short_dataset",
        "generated_speech/GPT_XTTS_v2.0_Full_9_2_SNF/snf_short_dataset"
    ]

    datasets = [load_from_disk(ds) for ds in dataset_paths_short]

    df_he_eval = pd.read_csv("processing/data/human_eval_snf.csv", sep=",")
    end_idx = args.group * args.num_samples
    start_idx = end_idx - args.num_samples
    print(f"{start_idx} -> {end_idx}")

    subset = df_he_eval[start_idx:end_idx]
    # Create a set of valid (dialect, sentence_id) pairs
    valid_pairs = set(zip(subset["dialect_region"], subset["sentence_id"]))


    datasets = [ds.filter(lambda x: (x["dialect"], x["sample_id"]) in valid_pairs) for ds in datasets]

    for ds_idx, _ in enumerate(dataset_paths_short):
    ...
    result = run_synthesis_evaluation_app(
        ...
        dataset_labels=["SNF_short_coqui_srf", "SNF_short_coqui_srf_finetuned", "SNF_short_coqui_base"],
