...
x = []
for text in texts:
    x.append(len(nlp(text)))

y = {"reference": texts, "token_count_ref": x}
df = pd.DataFrame(y)
df.to_csv("snf_long_sentences.csv")
...
for tid, text in enumerate(texts):
    for dial_tag in dial_tags:
        ...
        filtered_df = df[(df["dialect_region"] == LANG_MAP[dial_tag]) & (df["sentence"] == dialect_text)]
...
with open(os.path.join(OUT_PATH, XTTS_MODEL_TRAINED, TEXT_METADATA_FILE), "wt", encoding="utf-8") as f:
    for idx, text in enumerate(texts):
        dialect_text = text.split(". ")[0] + "."
        text_id = df[df["sentence"] == dialect_text]["sentence_id"].iloc[0]
        f.write(f"{idx}\t{text}\t{text_id}\n")

shutil.copytree(GENERATED_SPEECH_PATH,
                os.path.join(CLUSTER_HOME_PATH, GENERATED_SPEECH_FOLDER, XTTS_MODEL_TRAINED, GENERATED_SPEECH_FOLDER + "_snf_long"),
                dirs_exist_ok=True)

shutil.copy2(os.path.join(OUT_PATH, XTTS_MODEL_TRAINED, TEXT_METADATA_FILE),
             os.path.join(CLUSTER_HOME_PATH, GENERATED_SPEECH_FOLDER, XTTS_MODEL_TRAINED, TEXT_METADATA_FILE.replace(".txt", "_snf_long.txt")))
