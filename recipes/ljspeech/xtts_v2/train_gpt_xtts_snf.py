...
# CURRENT_CHECKPOINT_NAME = "GPT_XTTS_v2.0_LJSpeech_FT-January-10-2025_01+44PM-2ea5e850"
...
# XTTS_CHECKPOINT = f"{OUT_PATH}/{CURRENT_CHECKPOINT_NAME}/checkpoint_50000.pth"  # model.pth file
...
# Normal / All Samples
# txt_files = [f for f in os.listdir(DATASETS_PATH) if f.endswith('.txt') and not f.startswith(("ch_", "token_counted_", "more_than_6_"))]
txt_files = [f for f in os.listdir(DATASETS_PATH) if f.endswith('.txt')]

# More than 6 Tokens
# txt_files = [f for f in os.listdir(DATASETS_PATH) if f.endswith('.txt') and f.startswith("more_than_6_")]

# Training on CH-DE text
...
for metadata in txt_files:
    ...
    dialect_name = metadata.split("_")[0]
